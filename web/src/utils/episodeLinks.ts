...
const appleCache = new Map<string, Promise<Record<string, string>>>();
const pocketCastsCache = new Map<string, Promise<Record<string, string>>>();
const spotifyCache = new Map<string, Promise<Record<string, string>>>();
...
export async function getAppleEpisodeLinks(appleShowUrl: string): Promise<Record<string, string>> {
  if (appleCache.has(appleShowUrl)) {
    return appleCache.get(appleShowUrl)!;
  }
  ...
  appleCache.set(appleShowUrl, promise);
  return promise;
}
...
export async function getPocketCastsEpisodeLinks(pocketCastsUrl: string): Promise<Record<string, string>> {
  if (pocketCastsCache.has(pocketCastsUrl)) {
    return pocketCastsCache.get(pocketCastsUrl)!;
  }
  ...
  pocketCastsCache.set(pocketCastsUrl, promise);
  return promise;
}
...
export async function getSpotifyEpisodeLinks(spotifyShowUrl: string): Promise<Record<string, string>> {
  if (spotifyCache.has(spotifyShowUrl)) {
    return spotifyCache.get(spotifyShowUrl)!;
  }
  ...
  spotifyCache.set(spotifyShowUrl, promise);
  return promise;
}
